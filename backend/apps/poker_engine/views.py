...
class HandStatsView(views.APIView):
    ...
    The arithmetic lives in ``poker_engine.stats`` so the instructor
    dashboard's per-student drill-down computes identical numbers from the same
    code path rather than a second implementation that could drift.
    """
    permission_classes = (permissions.IsAuthenticated,)

    def get(self, request, *args, **kwargs):
        return Response(
            hand_stats_for(request.user), status=status.HTTP_200_OK,
        )
